# Legacy AWS Trial Expiry Reminder

> Archived reference only. This unauthenticated, workspace-unscoped Lambda/DynamoDB application is
> not part of the new runtime and must not be exposed alongside it.

A legacy serverless application that reminds you before a free trial turns into a paid
subscription. Eligibility, quotas, and charges depend on the account and current provider terms:
...
| `sentAt`        | String | ISO timestamp, set after SMTP submission        |
...
Gmail applies account-specific sending limits. SMTP acceptance does not prove recipient delivery.
...
This archive does not make a cost guarantee. Check current AWS and Gmail terms, account eligibility,
networking, quotas, and billing alarms before any use.
